    ...
    extends Controller
{
        ...
        if (isset($_POST['id']) && isset($_POST['title']) && isset($_POST['lead'])) {
            if (is_numeric($_POST['id'])) {
                ...
                try {
                    $article->fill($_POST);
                    $article->save();
                    header('Location: http://localhost/PHP-2-Professional-programming-main/Lesson7/Admin');
                    exit();
    ...
    protected function actionDelete()
    {
        if (!empty($_GET['id'])) {
            ...
            if (empty($article)){
                header('Location: http://localhost/PHP-2-Professional-programming-main/Lesson7/Admin/Default');
            }
